...
| P5 docs, QA, deploy | Drafted `docs/success-criteria.md`: for each of the eight questions, where it is answered on the page, what the reader sees on golden case 1 read from the running prototype, the table's scanning version, and the stories and tests that prove it. Criterion 8 is recorded as answered by decision (D9), not by the page | The rationale is mine to write, from `decisions.md` and this file. The agent may draft; I rewrite. Success criteria go into `docs/success-criteria.md` with where each is answered on the page |
